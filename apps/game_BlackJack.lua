...
function C4eT_BblugpblIIIa(cyMMa)
	local BblugPblIII = 0
	if cyMMa >= 500 then
		BblugPblIII = 150
	elseif cyMMa >= 250 then
		BblugPblIII = 50
	elseif cyMMa >= 100 then
		BblugPblIII = 30
	elseif cyMMa >= 75 then
		BblugPblIII = 25
	elseif cyMMa >= 50 then
		BblugPblIII = 20
	elseif cyMMa >= 25 then
		BblugPblIII = 10
	elseif cyMMa >= 10 then
		BblugPblIII = 4
	elseif cyMMa >= 5 then
		BblugPblIII = 2
	elseif cyMMa >= 1 then
...
local function drawDisplay()
    ...
    gpu.set(5, 17, '3. При удвоении ставки,')
    gpu.set(5, 18, '   выйгрыш возрастает!')
    gpu.set(5, 19, 'Правила игры есть в гугле.')
...
local function dialerStartPlay()
    ...
    if countCards(dialer_cards, true) > 21 then
        ...
        casino.reward(value + C4eT_BblugpblIIIa(value))
        message("Вы выиграли " .. C4eT_BblugpblIIIa(value) + value .. "$")
        os.sleep(time_sleep_end)
        login = false
        drawDisplay()
...
local function startGame()
    ...
    if (countCards(players_cards, true) == 21) then
        if (countCards(dialer_cards, true) == 11) then
            ...
            message("Вы выиграли " .. value + C4eT_BblugpblIIIa(value))
            casino.reward(value + C4eT_BblugpblIIIa(value))
            os.sleep(time_sleep_end)
            login = false
            drawDisplay()
